...
- `a` étant sous la touche morte, ses variantes accentuées ne peuvent pas être sur cette colonne sous peine d'un SFB important. `à` et `â` ont donc été placés à droite sur les lettres avec lesquelles il fait le plus petit SFB.

- `é` est de loin l'accent le plus important, mais est fortement contraint par le digramme `ée`. Le placer sur l'index rend ce digramme peu confortable en plus d'un SFB important de `é,`. Le placer sur `y` pour faire un roulement surcharge trop l'auriculaire en plus de ne pas être spécialement confortable. Du côté des consonnes, `é` fait étonnement peu de digrammes avec la colonne du `r`, il est donc derrière cette lettre. Cela contribue à de nombreux inrolls. La charge auriculaire sur l'auriculaire reste très correcte, mais le prix à payer est un SKU additionnel.
